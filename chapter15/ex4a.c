...
int main(void) 
{
	...
	/* Read line per line using fgets() and print to stdout using fprintf() */
	while (!feof(file1) || !feof(file2)) {  /* Test for end-of-file condition */
		fprintf(stdout, fgets(line, 80, file1));  /* extract and print lines */
		fprintf(stdout, fgets(line, 80, file2)); 
	}
